...
(function () {
  ...
  async function diagnoseLocally(f) {
    ...
    // ★ 살아 있는 텍스트가 «있다» 고 해서 거절하지 않는다.
    //
    // 2026-09-01, 실사용 제보로 고친 자리다. 예전에는 이랬다:
    //
    //     if (fontRes > 0 && (textOps > 0 || btOps > 0)) → "이미 살아 있습니다" (거절)
    //
    // 그런데 실무 인쇄 파일은 헤드라인만 윤곽선을 따고 본문은 살려 두거나, 텍스트 블록
    // 하나만 아웃라인인 경우가 **가장 흔하다.** 그 파일을 돌려보내면 정작 복원이
    // 필요한 고객을 문 앞에서 쫓아낸다.
    //
    // 그럼 브라우저에서 «아웃라인이 얼마나 있는지» 세면 되지 않나 — 안 된다.
    // 시도해 봤고 실패했다. 파일 바이트에서 패스 연산(f/c/v/y)을 세는 방식은
    // `.ai` 의 AIPrivateData 가 한 글자 연산자를 대량 오탐시켜 무너진다.
    // 실측: 전부 라이브인 원본 .ai 에서 «글자 도형» 이 956,878개로 나왔다
    // (같은 방식으로 진짜 혼합 파일은 49,086개 — 즉 순서가 뒤집힌다).
    // 페이지 콘텐츠 스트림만 골라 세려면 PDF 객체 그래프를 파싱해야 하는데,
    // 그건 브라우저에서 할 일이 아니다.
    //
    // 그래서 브라우저는 **판단하지 않는다.** 서버 probe 는 실측 0.06초에 정확히
    // 판정하고, 접수 직전에 그 판정을 본다 (`_refuse_if_nothing_to_restore`).
    // 복원할 것이 없으면 몇 초 안에 «결제하지 마십시오» 로 돌려보낸다.
    // 여기서 어림짐작으로 막는 것보다 그게 정확하고, 무엇보다 고객을 쫓아내지 않는다.
    if (hasLive) {
      return render("ok", "살아 있는 텍스트가 함께 있는 파일입니다",
        "서체 정보와 텍스트 명령이 남아 있습니다. 두 경우가 있습니다.<br>" +
        "<b>① 일부만 아웃라인</b> — 인쇄 파일에서 가장 흔한 형태입니다. " +
        "이미 살아 있는 글자는 그대로 두고, <b>도형이 된 글자만</b> 되살립니다.<br>" +
        "<b>② 전부 살아 있음</b> — 복원이 필요 없습니다. " +
        "이 경우 서버가 <b>몇 초 안에 그렇게 알려드리고 작업하지 않습니다.</b><br>" +
        "둘 중 어느 쪽인지는 브라우저에서 정확히 가릴 수 없어 서버가 판정합니다. " +
        "어느 쪽이든 <b>요금은 발생하지 않습니다.</b>", facts);
    }
  ...
  async function startRestore(file) {
    ...
    try {
      ...
      // ★ 복원 대상 판정은 «둘» 이다. 서버의 RESTORABLE_VERDICTS 와 같은 집합이어야 한다.
      //   2026-09-01 까지 여기가 "retypeset_required" 하나였다. 그래서 헤드라인만
      //   아웃라인인 파일 — 실무에서 가장 흔한 형태 — 이 진단을 통과하고도 여기서 막혔다.
      //   사용자가 «일부만 살아 있는데 살아있다면서 안 된다» 고 보고한 지점이다.
      if (d.verdict !== "retypeset_required" && d.verdict !== "partially_outlined") {
        return failBox("이 파일은 복원 대상이 아닙니다",
          (d.result && (d.result.title || d.result.detail)) ||
          "이미 편집 가능한 텍스트가 있거나, 이 방식으로 처리할 수 없는 형식입니다.");
      }
      if (d.verdict === "partially_outlined") {
        $("s3title").textContent = "일부만 아웃라인 — 도형이 된 글자만 복원합니다";
      }
